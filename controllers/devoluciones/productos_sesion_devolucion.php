...
//Primero vamos a comprobar que los productos que ya hay en la variable global pertenezcan a este pedido
...
//Si está vacía también la eliminamos
if (isset($_SESSION["devolucion"]) && empty($_SESSION["devolucion"])) unset($_SESSION["devolucion"]);
    ...
    foreach ($productosPedido as $item) { //Recorremos los productos del pedido
        ...
        foreach ($productosDevueltos as $devueltoItem) {
            if ($devueltoItem['id'] == $item['id'] && $devueltoItem['talla'] == $item['talla']) { //Si encuentra el producto
                if ($devueltoItem['cantidad_total'] < $item['cantidad']) { //Y aun no está devuelta toda la cantidad de ese producto
                    //Vamos a calcular la cantidad máxima que se puede devolver
                    $cantidad_max = $item['cantidad'] - $devueltoItem['cantidad_total'];
                } else {
